...
if (($qus_pdf_count>4)&&is_readable(FCPATH.$pdf_file_path_common.$common_pdf)) {
               ...
               echo "<ul class='nav nav-pills'><li><a class='btn btn-raised btn-lg btn-warning' href='".base_url('online-test/getresult/'.encrypt($common_pdf.'c-h-e-c-k'.$this->session->userdata('customer_id')))."' target='_blank'>Download Solution File</a></li></ul>";
}
} 
...
if (($qus_pdf_count>4)&&is_readable($pdf_file_path.$qus_pdf)) {
               echo "<ul class='nav nav-pills'><li><a class='btn btn-raised btn-lg btn-warning' href='".base_url('online-test/getresult/'.encrypt($qus_pdf.'c-h-e-c-k'.$this->session->userdata('customer_id')))."' target='_blank'>Download Question File</a></li>";
}
...
$solution_pdf_count=strlen($solution_pdf);
 if (($solution_pdf_count>4)&&is_readable($pdf_file_path.$solution_pdf)) {
               echo "<li><a class='btn btn-raised btn-lg btn-info' href='".base_url('online-test/getresult/'.encrypt($solution_pdf.'c-h-e-c-k'.$this->session->userdata('customer_id')))."' target='_blank'>Download Solution File</a></li></ul>";
 }
 } 
?>
</div><br>
