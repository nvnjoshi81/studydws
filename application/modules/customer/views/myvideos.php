    ...
    <div class="container" id="container_top">
        ...
        <div class="btn-success" style="padding:10px 15px;">
                    <h4 class="panel-title">
                        <a data-toggle="collapse" data-parent="#accordion1" href="#collapseThree"><i class="material-icons">assignment_returned</i> &nbsp; My Video Packages Subscription</a>
                    </h4> 
        </div>
                <div id="collapseThree" class="panel-collapse collapse">
                    ...
                    if (isset($purchased_videos) && count($purchased_videos) > 0) {               
            $itemdetail=NULL;
                   $hrefbuy_fullpackages=''; 
             ?>    <div class="panel-body"><?php 
                                foreach ($purchased_videos as $k => $v) {                                $i_cnt=1;
                                    $content = '';
                                    foreach ($v as $k1 => $v1) {
                                        if ($k == 2) {
                                            $content = 'videos';
            ...
            $videoProduct= $this->Examcategory_model->getProductVideo($exam_id,$record->sid,$record->cid);
                    ...
                    if ($subject_id > 0 && $subject_id == $record->sid) {
                        ...
                        $sm = $this->Videos_model->getVideosCount($exam_id, $record->sid, $record->cid);
...
$hrefbuy_fullpackage = $content_fullpackages.'/' . url_title($details->exam, '-', true) . '/' . $details->exam_id;
     ...
     ?>
                              ...
                              ?>	
  <label class="btn btn-success active"  onclick="showbought_videos('<?php echo url_title($mainExamName); ?>',<?php echo $mainExamid; ?>,'<?php echo url_title($pkey); ?>',<?php echo $pvalue['id']; ?>,'<?php echo url_title($chaptArr[1]); ?>','<?php echo $chaptArr[0] ?>')">
...
<?php       
     ...
     ?>
                                    ...
                                    <div class="well well-sm text-center my-5"><?php 
                                        foreach($purchased_data_array[$pkey]['chapters'] as $chaptArr){  ?>	
  <label class="btn btn-success active"  onclick="showbought_videos('<?php echo url_title($url_examname); ?>',<?php echo $url_examid; ?>,'<?php echo url_title($pkey); ?>',<?php echo $pvalue['id']; ?>,'<?php echo url_title($chaptArr[1]); ?>','<?php echo $chaptArr[0] ?>')">
    <button class="btn btn-sm btn-outline-secondary" type="button"><?php  echo $chaptArr[1]; ?></button>
  </label>
             ...
             ?>  <div class="panel-body">There is some problem please try again.</div><?php 
          ...
          <?php   $i_cnt++;
             ...
             ?>  <div class="panel-body">You have not purchased any Video package.</div><?php 
