...
// get purchased products            
            $products=$this->Customer_model->getCustomerProucts($customer_id);
                $purchased=array();
               if($products){
...
<!-- row well well-sm well-lg-->
    ...
    <div class="col-xs-12 col-md-12 text-center">
            <span class="ts-btn">
            <h3>In Order to view and download Result, Question paper, Answers-key, Solution and Analytical Reports <button type="button" class=" btn-md btn btn-success btn-raised btn-lg">You Need To Buy This Series.</button>
            </h3>
            </span> 
    </div>
    </div>      
...
if (($qus_pdf_count>4)&&is_readable(FCPATH.$pdf_file_path_common.$common_pdf)) {
               echo "<li><a class='btn btn-raised btn-lg btn-warning' href='".base_url('online-test/getresult/'.encrypt($common_pdf.'c-h-e-c-k'.$customer_id))."' target='_blank'>Download Solution File</a></li>";
} ?></ul>
</div><br><?php
} 
				 }
...
if (($qus_pdf_count>4)&&is_readable($pdf_file_path.$qus_pdf)) {
               echo "<li><a class='btn btn-raised btn-lg btn-warning' href='".base_url('online-test/getresult/'.encrypt($qus_pdf.'c-h-e-c-k'.$customer_id))."' target='_blank'>Download Question File</a></li>";
}
$ans_pdf_count=strlen($ans_pdf);
 if (($ans_pdf_count>4)&&is_readable($pdf_file_path.$ans_pdf)) {              
               echo "<li><a class='btn btn-raised btn-lg btn-success'href='".base_url('online-test/getresult/'.encrypt($ans_pdf.'c-h-e-c-k'.$customer_id))."' target='_blank'>Download Answer File</a></li>";
 }
$solution_pdf_count=strlen($solution_pdf);
 if (($solution_pdf_count>4)&&is_readable($pdf_file_path.$solution_pdf)) {
               echo "<li><a class='btn btn-raised btn-lg btn-info' href='".base_url('online-test/getresult/'.encrypt($solution_pdf.'c-h-e-c-k'.$customer_id))."' target='_blank'>Download Solution File</a></li>";
 }
?>          
</ul>
</div><br>
                ...
                <div role="tabpanel" class="tab-pane active" id="overall">
                    ...
                    <table class="table table-hover table-responsive">
                        ...
                        <tr>
                            ...
                            <td><a href="<?php echo base_url('apponline-test/userresult/'.$allrow->id); ?>">
                            <?php if ($cc == 1) { ?>
                ...
                <div class="col-md-12 col-md-8 nopadding"  style="min-height: 375px;border-left:solid 1px #ccc;">
                    <div class="row  col-md-12">
                        <div class="col-sm-4">
                            ...
                            if($obtain_marks>0){
                            echo round(($obtain_marks/$total_marks)*100,2); }
                            ?>%
                    ...
                    <div class="row col-md-12">
                        <div class="col-xs-10 col-xs-offset-1">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                   <div class="progress">
                                        <div class="progress-bar progress-bar-warning" role="progressbar" aria-valuenow="<?php echo $usertest_result_info->time_taken; ?>" aria-valuemin="0" aria-valuemax="<?php echo $usertest_result_info->time_taken+$usertest_result_info->time_remaining;?>" style="width: <?php echo ($usertest_result_info->time_taken/($usertest_result_info->time_taken+$usertest_result_info->time_remaining))*100; ?>%">
                                    <span class="sr-only"><?php echo ($usertest_result_info->time_taken/($usertest_result_info->time_taken+$usertest_result_info->time_remaining))*100; ?>% Complete (success)</span>
                                    </div>
                                    </div>     
                                </div>
                                <div class="col-sm-1">
...
//print_r($sections); ?>
          <thead>
            ...
            <tr class="oaerror <?php if($value1['answered_correctly']==1){ echo 'success'; }elseif($value1['answered_correctly']==2){ echo 'warning';}elseif($value1['answered_correctly']==0){ echo 'danger';}?>">
                <td>Question <?php echo $ucc.')' .$value1['question']; ?></td>
