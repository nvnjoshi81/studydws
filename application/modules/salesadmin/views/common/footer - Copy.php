 ...
 //Redirect user if user is not logged in.
  if($_SESSION['first_name']==''){ 
      $dir_salesadmin=$this->config->item('dir_salesadmin');
       redirect('/'.$dir_salesadmin.'/logout');
   }
...
<?php 
if($this->session->userdata('saleslogged_in')){ ?>
...
function mergeSection(is_final){
            ...
            $.ajax({
                ...
                success:function(response) {
                    if(response.count > 0){
                        ...
                        trHTML += '<option value=' + item.id + '>' + stripHTML(item.name) + '</option>';
                        
                        });
                        trHTML += '</select>';
                        trHTML += '<input type="hidden" name="related_moduleType" value="'+related_moduleType+'" >&nbsp;                    <input type="checkbox" name="mergeDelete" value="Yes" >&nbsp;Delete?(Select Checkbox to delete The Value.)';
   ...
   function relateQuestion(question_id,chapter_id){
    ...
    /*Add to Cart For franchise customer */
    
     $(document).on("click", ".addtocart", function() {
        var e=$(this).attr("itemqty"), r=$(this).attr("offline"), a=$(this).attr("action_type"),usrid=$(this).attr("usrid");
        $("#quantity").get(0)&&(e=$("#quantity").val()), addtocart($(this).attr("itemid"), e, $(this).attr("itemprice"), $(this).attr("itemname"), "", r, a,usrid)
    }
    )
    
    function addtocart(e, r, a, t, i, s, n,usrid=0) {
    $.ajax( {
        url:base_url+"api/cart/additem", type:"POST", async:!0, data:"id="+e+"&qty="+r+"&price="+a+"&name="+t+"&weight="+i+"&offline="+s+"&usrid="+usrid+"&franchiseid=<?php echo $franchiseid ;?>", success:function(i) {
