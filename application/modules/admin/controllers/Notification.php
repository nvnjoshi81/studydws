...
class Notification extends MY_Admincontroller {
    ...
    public function index($page = 0) {
        ...
        $config["total_rows"] = $this->Notification_model->getMediaCount();
        ...
        $this->data['media'] = $this->Notification_model->getcontent();
		$this->load->model('Examcategory_model');
        $this->data['media_array'] = $this->Notification_model->getmedia();      
    ...
    public function add() { 
        ...
        $this->data['content'] = 'notification/add';
        $this->load->view('common/template', $this->data);
    }
...
if(isset($date)&&$date!=''){
	...
	$date = date("F j, Y"); 
        ...
        $this->Notification_model->createMedia($media_data);         
     ...
     public function edit($media_id) {   
        ...
        $this->data["links"] = $this->pagination->create_links();
        $this->data['media'] = $this->Notification_model->getcontent();
            $mediaResult = $this->Notification_model->getMediaById($media_id);
            $this->data['mediaResult']=$mediaResult;
            $this->data['content'] = 'notification/edit';
            $this->load->view('common/template', $this->data);         
     }
	...
	$date = date("F j, Y");
        ...
        $this->Notification_model->editMedia($media_data,$id);
        $this->session->set_flashdata("message","Information Updated Successfully.");
        redirect('admin/notification/edit/'.$id);
         
     }
     
     public function delete($id){
        $this->Notification_model->deleteMedia($id);
        $this->session->set_flashdata("message","Information Deleted Successfully.");
        redirect('admin/notification');
     }
	 
     public function notify($id){
        $mediaResult = $this->Notification_model->getMediaById($id);		
		...
		$notifyInfoArray = $this->Notification_model->getStudentByClassId($selectedclass);
		
		
		die(' you are on line 213 - \application\modules\admin\controllers/Notification.php');
 ...
 //echo $contents;
     ...
     public function androidnotify(){
		 ...
		 $this->data['content'] = 'notification/appnotify';
            $this->load->view('common/template', $this->data);  
