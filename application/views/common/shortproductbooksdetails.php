...
<div class="row">
    ...
    <div class="col-xs-12 col-md-2">
        ...
        if (!$user_purcheses || !$pid_value) {         
           ...
           $itemdetail = $this->File_model->getBooksDetails($isProduct->item_id);
            ?>
        <a href="<?php echo base_url('books/download/'.encrypt($itemdetail->id.'.'.$this->session->userdata('customer_id')))?>" class="subjectbtn btn btn-primary btn-raised btn-lg">Download Now</a>
            <?php 
...
?>     
    ...
    if($userBrought=='yes'){
        ?> <a target="_blank" href="<?php echo base_url('books/download/'.encrypt($file->id.'.'.$this->session->userdata('customer_id')))?>"  class="subjectbtn btn btn-primary btn-raised btn-lg" type="button">
