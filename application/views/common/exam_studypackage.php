...
<div class="Clearfix"></div> 
        <div class="row">
			...
			$studypackageimage=$this->config->item('bgimages');
            if ((count($productslist) > 0) && ($productslist[0] != '')) {
                $count = 0;
		 ...
		 ?>
            ...
            <?php } ?>
								...
								<?php if ($product->type == '2') {
                                           ...
                                           if($prdhead_cnt>50){
                                               $prdhead=substr($prdhead_raw,0,60).'...';
                                           } else{
                                               if($prdhead_cnt<30){
                                        ...
                                        ?>
                                        <div style="position: absolute;top: 16px;right: 1px;color:white;" ><h5 style="color:white;" class="vid_prod_hed" <?php echo $margincss; ?> title="<?php echo $prdhead_raw; ?>"><?php echo $prdhead;?></h5></div>       
                            </div>
                            <div class="info">
 ...
 if($product->type == '2') {
	...
	?>										
<a href="<?php echo base_url('purchase-courses#urlid_'.$exam_id)?>" class="subjectbtn btn btn-success  btn-raised btn-lg btn-sm btn-md">Buy Now</a>
												<!--<button itemname="<?php echo $product_name; ?>" 
                                                        ...
                                                        offline='0'
                                                        action_type="1"
                                                        class="btn-md btn btn-raised btn-success addtocart" name="btnAddToCart">Buy Now</button>-->
                                            <?php } } else {
                                                    ?>
                                                <?php
                                                if($product->type == '2') { ?><a href="<?php echo getProductLink($product, $type); ?>"><button class="btn-lg btn-sm btn-md btn btn-raised btn-success" name="btnAlreadyExist">Watch Now</button></a>
                                                <?php 
                                                }else{ 
                                         if(isset($cnttimes)&&$cnttimes>0) {
                                             ...
                                             $textdownload='Download now';
                                                    ...
                                                    ?>
                                            <a href="<?php echo base_url('study-packages/download/'.encrypt($product->file_id.'.'.$this->session->userdata('customer_id')))?>" target="_blank">
                                            <button class="btn-lg btn-sm btn-md btn btn-raised btn-success" name="btnAlreadyExist"><?php echo $textdownload.$cnttimes; ?></button>
                                            </a> 
                                                 <?php
                                                 } ?>
                                            <?php }
                                ...
                                <?php } else {
                                ?>
                            <button class="btn-lg btn-sm btn-md btn btn-raised btn-success">NOT FOR SALE</button>
                                <?php }
                                ?>
                            </div>
                        </div>
