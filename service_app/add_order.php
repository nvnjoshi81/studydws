<?php
	...
	$obs = mysqli_query($conn, $query_req1s);
	//adding a video product for the bigger study package product
	$getp = "select * from cmspricelist where id = '$dproductid'";
    $qgetp = mysqli_query($conn, $getp);
    while($mfagetp = mysqli_fetch_array($qgetp))
    { 
    $exam_id = $mfagetp['exam_id'];
    $qavp = "SELECT * FROM cmspricelist WHERE exam_id = '$exam_id' and subject_id = '0' AND chapter_id = '0' and type = '2'";
    {
        $mqavp = mysqli_query($conn, $qavp);
        while($mfaqavp = mysqli_fetch_array($mqavp))
        {
            $vpro_id = $mfaqavp["id"];
            $query_req1s="INSERT INTO cmsorder_details (order_id,price,product_id,quantity,type,user_id,end_date) VALUES ('$ooip','$prices','$vpro_id','$quantitys','$typee','$user_id',$tim)"; 
	        $obs = mysqli_query($conn, $query_req1s);
        }
    }
    //adding a test series product for the bigger study package product
    $qavp = "SELECT * FROM cmspricelist WHERE exam_id = '$exam_id' and subject_id = '0' AND chapter_id = '0' and type = '3'";
    {
        $mqavp = mysqli_query($conn, $qavp);
        while($mfaqavp = mysqli_fetch_array($mqavp))
        {
