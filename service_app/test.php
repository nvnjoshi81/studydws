<?php
 ...
 // echo "SELECT * FROM cmsorders join cmsorder_details on cmsorders.id =  cmsorder_details.order_id join cmspricelist on cmspricelist.id=cmsorder_details.product_id  where  cmsorders.user_id = '$user_id' and cmspricelist.type='2'";
	$result = mysqli_query($conn,"SELECT * FROM cmsorders join cmsorder_details on cmsorders.id =  cmsorder_details.order_id join cmspricelist on cmspricelist.id=cmsorder_details.product_id  where  cmsorders.user_id = '$user_id' and cmspricelist.type='2'" );
...
if($subTmp){$tmp['status'] = "success";$tmp['datatwo'] = $subTmp; }
	...
	function getmarvelcategoryn($mar_id,$conn) {		
		...
		$result = mysqli_query($conn,"SELECT * FROM cmspricelist where id = '$mar_id' and type = '2'");
		if($rows = mysqli_fetch_array($result)) 
		{
			...
			$returnValue['image'] = "http://dev.hybridinfotech.com/assets/frontend/product_images/studypackage_blank.png";
		}
		return $returnValue;
	}
