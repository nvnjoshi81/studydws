...
if (!empty($_POST['email']) && !empty($_POST['password'])&& !empty($tes_id) ) {
	...
	while($ree = mysqli_fetch_array($opd))
	{
		...
		{
			//echo "all Right";
      echo $query = "select * from cmscustomers where email='".$email."' and password='".$password."' and device_id='".$device_id."'";
	...
	if($dbuserkey>"")
	{
	    $up = "UPDATE cmscustomers SET device_id = '$device_id', subject_id = '28' where email = '$email' AND password = '$password'";
	}
	else
	{
	$up = "UPDATE cmscustomers SET device_id = '$device_id', subject_id = '28', user_key = '$userkey' where email = '$email' AND password = '$password'";
	}
    ...
    {
	$myuserd = $row['id'];	
	$data = array("response"=>array("status" => "true","username" => $row["firstname"],"email" => $row["email"], "user_id" => $row["id"],"category_id" => $row["subject_id"],"user_key" => $row["user_key"], "verified" => $row["status"], "msg" => "Success"));
	}
    }
	}
