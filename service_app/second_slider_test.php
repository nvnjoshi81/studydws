<?php 
        ...
        while($row = mysqli_fetch_array($result)) {
			 $mar_id = $row['id'];
			$job = array();
			echo $qryusrsrch = "SELECT * FROM cmsorders INNER JOIN cmsorder_details on cmsorders.id = cmsorder_details.order_id WHERE cmsorders.user_id = '$user_id' AND cmsorder_details.product_id = '$mar_id' AND cmsorders.status = '1'";
