...
if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($tes_id)) 
  ...
  $passwordn = '';
   ...
   $dtnow = strtotime("now");
   $in = "INSERT INTO cmscustomers (firstname,email,device_id,user_key,subject_id,is_app_registered, created_dt, usertype, is_social) VALUES ('$name','$email','$device_id','$user_key','0','1','$dtnow', 'student', '1')";
    ...
    {
    $data = array("response"=>array("status" => "false", "msg" => "Did you changed your Device? if that is the case please contact us at contact@studyadda.com Else we do not allow access on multiple devices.", "email" => $emails,"username" => $firstname,"category" => $category, "category_name" => $category_name,"user_id" => $re_id,"user_key" => $userc["user_key"], "verified" => "1"));           
    }
    }
    }
    else 
