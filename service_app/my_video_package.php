...
if($get_api){
    	...
    	$resultp = mysqli_query($conn,"SELECT * FROM cmsorders where user_id = '$user_id' and status = '1'");
		if($rowp = mysqli_fetch_array($resultp)) 
		{ $order =  $rowp['id'];	}
	$result = mysqli_query($conn,"SELECT * FROM cmsorder_details where order_id = '$order' and type = '2'");
}
...
if($subTmp){$tmp['status'] = "success";$tmp['data'] = $subTmp; }
	...
	/*	echo "SELECT * FROM cmsorder_details join cmscart_items on cmsorder_details.product_id=cmscart_items.product_id where cmsorder_details.id = '$mar_id'	and cmscart_items.end_date > '$tim'" ;
		...
		*/
		$result = mysqli_query($conn,"SELECT * FROM cmsorder_details join cmscart_items on cmsorder_details.product_id=cmscart_items.product_id where cmsorder_details.id = '$mar_id' and cmsorder_details.type = '2'  and cmscart_items.end_date > '$tim'");
