...
if($get_api){
		...
		{ $order =  $rowp['id'];	}
	$result = mysqli_query($conn,"SELECT * FROM cmsorder_details where order_id = '$order' and type = '3'");
}
...
if($subTmp){$tmp['status'] = "success";$tmp['data'] = $subTmp; }
	...
	/*	echo "SELECT * FROM cmsorder_details join cmscart_items on cmsorder_details.product_id=cmscart_items.product_id where cmsorder_details.id = '$mar_id'	and cmscart_items.end_date > '$tim'" ;
		...
		*/
		$result = mysqli_query($conn,"SELECT * FROM cmsorder_details join cmscart_items on cmsorder_details.product_id=cmscart_items.product_id where cmsorder_details.id = '$mar_id' and cmsorder_details.type = '3'  and cmscart_items.end_date > '$tim'");
