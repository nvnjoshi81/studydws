...
if (!empty($_POST['otp'])&& !empty($tes_id)) {
 ...
 $query = "select * from cmscustomers where otp='".$otp."' and (email = '$email' or mobile = '$contact')";
    $result = mysqli_query($conn,$query);
    if (mysqli_num_rows($result) > 0) {
        $newarray = array();
        if ($row = mysqli_fetch_array($result)) {
  
        $update_staus=mysqli_query($conn,"update cmscustomers set status='1' where otp='".$row['otp']."' and (email = '$email' or mobile = '$contact')");
