...
error_reporting(0);
    ...
    if($get_api)
    {
    $qry = "SELECT id FROM cmspackages_counter WHERE exam_id = '$category' AND subject_id = '$subject_id' AND level = 'subject'  AND total_package > '0' AND total_question > '0'";
...
if($subTmp){$tmp['status'] = "success";$tmp['data'] = $subTmp; }
	...
	function getmarvelcategory($mar_id,$conn) {		
		...
		$qry = "SELECT id, total_package, total_question, module_type FROM cmspackages_counter WHERE id = '$mar_id'";
		$result = mysqli_query($conn,$qry);
