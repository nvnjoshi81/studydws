...
<?php 
	...
	$result = mysqli_query($conn, "SELECT * FROM `postings` WHERE `subject_id`>'0' group by `category_id");
	while($row = mysqli_fetch_array($result)) {
			  $mar_id = $row['category_id'];
			$job = array();
    ...
    // echo "SELECT * FROM postings WHERE exam_id = '$iid' and category_id > '0'  GROUP BY subject_id ASC";
	$result = mysqli_query($conn,"SELECT * FROM postings WHERE category_id = '$iid' and subject_id > '0'  GROUP BY subject_id ASC");
    while($row = mysqli_fetch_array($result)) 
        	{
