...
<?php 
	...
	$result = mysqli_query($conn, "SELECT * FROM `cmsquestionbank_relations` where  exam_id > '0' and  subject_id > '0' GROUP BY `exam_id` ORDER BY exam_id DESC");
   ...
   //  echo "SELECT * FROM cmssamplepapers_relations WHERE exam_id = '$iid'  GROUP BY subject_id ASC";
	$result = mysqli_query($conn,"SELECT * FROM cmsquestionbank_relations WHERE exam_id = '$iid' and subject_id > '0'  GROUP BY subject_id ASC");
    while($row = mysqli_fetch_array($result)) 
        	{
