...
include('../../service_app/config.php');
  ...
  if($containtype==videos){ $containtype='2';}
   if($containtype==study-packages){ $containtype='1';}
if($subject_id > 0){ $subject_ids=$subject_id ;}else {$subject_ids="0";}   
 ...
 $result = mysqli_query($conn, "SELECT `cmspricelist`.`id` as `productlist_id`, `cmspricelist`.`exam_id`, `cmspricelist`.`subject_id`,
  ...
  `cmspricelist`.`type` = '$containtype' and `cmspricelist`.`subscription_expiry` >= '1' and `cmspricelist`.`subject_id`='$subject_ids' and `cmspricelist`.`chapter_id`='0' AND
